...
export default new Vuex.Store({
  state: {
    ...
    comedy: [
      ...
      },
      {
        ...
        price: 30,
        id: uuid(),
        performers: ["Amalia Georgescu", "Răzvan Darie", "Mădălina Ștefănescu"],
  ...
  mutations: {
    UPDATE_EVENTS_LIST(state) {
      return this.$store.getters.getAllEvents.filter((event) => {
        event.name.toLowerCase().includes(state.searchWord) ||
          event.location.toLowerCase().includes(state.searchWord);
      });
    },
  ...
  actions: {
    updateEventsList(context) {
      context.commit("UPDATE_EVENTS_LIST");
    },
