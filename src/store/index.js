...
export default new Vuex.Store({
      ...
      comedy: [
        {
          ...
          venue: "Comedy Club",
          price: 40,
          performers: ["Mihai Bobonete", "Vladimir Drăghia", "Smaranda Luna"],
        ...
        },
        {
          ...
          price: 35,
          performers: ["Andreea Perju", "Andrei Bădescu", "Gabriel Nedelea"],
        ...
        },
        {
          ...
          price: 45,
          performers: ["Costel Bojog", "Maria Popescu", "Andrei Runcanu"],
        ...
        },
        {
          ...
          price: 50,
          performers: ["Mihai Găinușă", "Sorin Brotnei", "Irina Popescu"],
  ...
  getters: {
    getAllEvents: (state) => {
      return state.events.festivals
        .concat(state.events.sports)
        .concat(state.events.comedy)
        .concat(state.events.kidsEvents);
    },
  },
  mutations: {
    UPDATE_EVENTS_LIST(state, searchWord) {
      state.events.festivals = state.events.festivals.filter((event) => {
        return (
          event.name.toLowerCase().includes(searchWord) ||
          event.location.toLowerCase().includes(searchWord)
        );
      });
      state.events.comedy = state.events.comedy.filter((event) => {
        return (
          event.name.toLowerCase().includes(searchWord) ||
          event.location.toLowerCase().includes(searchWord)
        );
      });
      state.events.sports = state.events.sports.filter((event) => {
        return (
          event.name.toLowerCase().includes(searchWord) ||
          event.location.toLowerCase().includes(searchWord)
        );
      });
      state.events.kidsEvents = state.events.kidsEvents.filter((event) => {
        return (
          event.name.toLowerCase().includes(searchWord) ||
          event.location.toLowerCase().includes(searchWord)
        );
  ...
  actions: {
    updateEventsList(context, searchWord) {
      context.commit("UPDATE_EVENTS_LIST", searchWord);
    },
  },
  modules: {},
});
